...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                const prevHeight = self.height;
                if (prevHeight === -1) {
                    block.previousBlockHash = "x0";
                } else {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (now - timestamp > 300000) {
                reject("Message older than 5min")
            }
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            self.chain.forEach(async block => {
                const decodedBlock = await block.getBData();
            ...
            for (let i = 0; i <= self.height; i++) {
                ...
                if (i < self.height){
                    ...
                    if (block.hash !== nextBlock.previousBlockHash) {
                        errorLog.push(`Chain ${i} is invalid`)
                    }
                }
            }
            resolve(errorLog)
